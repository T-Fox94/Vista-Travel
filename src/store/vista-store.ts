...
interface VistaState {
  ...
  setNotifBooking: (val: boolean) => void;
  setNotifPromo: (val: boolean) => void;
  setNotifPrice: (val: boolean) => void;
  setNotifReminder: (val: boolean) => void;
  setTwoFAEnabled: (val: boolean) => void;
  setCurrency: (currency: "USD" | "EUR" | "GBP" | "ZMW" | "ZAR") => void;
...
export const useVistaStore = create<VistaState>()(
  persist(
    (set, get) => ({
      ...
      getCurrencySymbol: () => {
        const { currency } = get();
        const symbols: Record<string, string> = { USD: "$", EUR: "€", GBP: "£", ZMW: "ZK", ZAR: "R" };
      ...
      setNotifBooking: (notifBooking) => set({ notifBooking }),
      setNotifPromo: (notifPromo) => set({ notifPromo }),
      setNotifPrice: (notifPrice) => set({ notifPrice }),
      setNotifReminder: (notifReminder) => set({ notifReminder }),
    ...
    }),
    {
      ...
      // Partialize to only persist relevant settings
        partialize: (state) => ({
          currency: state.currency,
          theme: state.theme,
          language: state.language,
          notifBooking: state.notifBooking,
          notifPromo: state.notifPromo,
          notifPrice: state.notifPrice,
          notifReminder: state.notifReminder,
          twoFAEnabled: state.twoFAEnabled,
          wishlist: state.wishlist,
          viewedTicketIds: state.viewedTicketIds,
          tickets: state.tickets,
          notifications: state.notifications,
          unreadNotificationCount: state.unreadNotificationCount,
          unreadSupportTicketCount: state.unreadSupportTicketCount,
        }),
    }
  )
);
